...
func TestParseProfileFeedOptionsRejectsInvalidLimit(t *testing.T) {
	...
	if !errors.Is(err, UserErrors.InvalidProfileFeedLimit) {
		t.Fatalf(
			"parseProfileFeedOptions() error = %v, want %v",
			err,
			UserErrors.InvalidProfileFeedLimit,
		)
	}
}
...
func TestProfileFeedOptionsRejectsUnknownSort(t *testing.T) {
	...
	if !errors.Is(err, UserErrors.InvalidProfileFeedSort) {
		t.Fatalf(
			"ProfileFeedOptions.Validate() error = %v, want %v",
			err,
			UserErrors.InvalidProfileFeedSort,
		)
	}
}
...
func TestProfileFeedOptionsRejectsInvertedFameRange(t *testing.T) {
	...
	err := options.Validate()
	if !errors.Is(err, UserErrors.InvalidProfileFeedFilter) {
		t.Fatalf(
			"ProfileFeedOptions.Validate() error = %v, want %v",
			err,
			UserErrors.InvalidProfileFeedFilter,
		)
	}
}
