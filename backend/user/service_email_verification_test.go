...
func TestServiceVerifyEmailHashesRawToken(t *testing.T) {
	...
	err := NewService(repository, &fakeImageStorage{}).VerifyEmail(
		context.Background(),
		rawToken,
	)
	if err != nil {
...
func TestServiceVerifyEmailRejectsBlankToken(t *testing.T) {
	err := NewService(&fakeUserRepository{}, &fakeImageStorage{}).VerifyEmail(
		context.Background(),
		"   ",
	)
	if !errors.Is(err, UserErrors.InvalidVerificationToken) {
		t.Fatalf("VerifyEmail() error = %v, want %v", err, UserErrors.InvalidVerificationToken)
	}
}
...
func TestServiceResendVerificationEmailWrapsDeliveryError(t *testing.T) {
	...
	if !errors.Is(err, UserErrors.EmailDeliveryFailed) {
		t.Fatalf("ResendVerificationEmail() error = %v, want %v", err, UserErrors.EmailDeliveryFailed)
	}
}
