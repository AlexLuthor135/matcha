...
func (h *Handler) UploadPhoto(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case errors.Is(err, ProfileErrors.PhotosMissing):
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	case errors.Is(err, ProfileErrors.PhotoEmpty):
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	case errors.Is(err, ProfileErrors.PhotoTooLarge):
		http.Error(w, "Each photo must not be larger than 5 MB", http.StatusRequestEntityTooLarge)
		return
	case errors.Is(err, ProfileErrors.PhotoTypeUnsupported):
		http.Error(w, "Photos must be JPEG, PNG or WEBP", http.StatusUnsupportedMediaType)
		return
	case errors.Is(err, ProfileErrors.PhotoLimitExceeded):
