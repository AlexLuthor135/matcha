...
func TestServiceUploadPhotosRejectsInvalidInput(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:      "no photos",
			filesData: nil,
			wantErr:   ProfileErrors.PhotosMissing,
		},
		{
			name:      "too many photos",
			filesData: validFiles,
			wantErr:   ProfileErrors.PhotoLimitExceeded,
		},
		{
			name:      "empty photo",
			filesData: [][]byte{nil},
			wantErr:   ProfileErrors.PhotoEmpty,
		},
		{
			name:      "photo too large",
			filesData: [][]byte{make([]byte, maxPhotoFileSize+1)},
			wantErr:   ProfileErrors.PhotoTooLarge,
		},
		{
			name:      "unsupported photo type",
			filesData: [][]byte{[]byte("plain text is not an image")},
			wantErr:   ProfileErrors.PhotoTypeUnsupported,
		},
	}
...
func TestServiceUploadPhotosValidatesEveryFileBeforeSaving(t *testing.T) {
	...
	if !errors.Is(err, ProfileErrors.PhotoTypeUnsupported) {
		t.Fatalf("UploadPhotos() error = %v, want %v", err, ProfileErrors.PhotoTypeUnsupported)
	}
	if saveCalls != 0 {
