...
func parseProfileFeedLimit(rawLimit string) (int, error) {
	...
	if err != nil {
		return 0, DiscoveryErrors.InvalidProfileFeedLimit
	}
	return limit, nil
}
...
func parseOptionalFeed[T int | int64 | float64](rawValue string) (*T, error) {
	...
	if err != nil {
		return nil, DiscoveryErrors.InvalidProfileFeedFilter
	}
	result := value.(T)
	return &result, nil
}
...
func (h *Handler) GetProfileFeed(w http.ResponseWriter, r *http.Request) {
...
func (h *Handler) serveProfileList(w http.ResponseWriter, r *http.Request, search bool) {
	...
	switch {
	case errors.Is(err, DiscoveryErrors.InvalidProfileFeedLimit) ||
		errors.Is(err, DiscoveryErrors.InvalidProfileFeedFilter) ||
		errors.Is(err, DiscoveryErrors.InvalidProfileFeedSort) ||
		errors.Is(err, DiscoveryErrors.InvalidLocation):
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	case errors.Is(err, DiscoveryErrors.UserNotFound):
