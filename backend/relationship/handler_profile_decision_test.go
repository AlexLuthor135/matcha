...
func TestSaveProfileDecisionHandlerRejectsInvalidRequests(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantStatus:   http.StatusBadRequest,
			wantResponse: RelationshipErrors.CannotDecideOwnProfile.Error() + "\n",
		},
		{
			...
			wantStatus:   http.StatusBadRequest,
			wantResponse: RelationshipErrors.InvalidProfileDecision.Error() + "\n",
		},
	}
...
func TestSaveProfileDecisionHandlerMapsRepositoryErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			repositoryErr: RelationshipErrors.TargetUserNotFound,
...
func TestSaveProfileDecisionHandlerDoesNotNotifyAfterBlockedError(t *testing.T) {
	repository := &fakeUserRepository{
		...
		saveDecisionFn: func(
			...
			return SaveProfileDecisionResult{}, RelationshipErrors.TargetUserNotFound
		},
	}
	notificationCalls := 0
...
func TestSaveProfileDecisionHandlerRequiresProfilePicture(t *testing.T) {
	...
	if response.Body.String() != RelationshipErrors.ProfilePictureRequired.Error()+"\n" {
		t.Fatalf(
			"body = %q, want %q",
			response.Body.String(),
			RelationshipErrors.ProfilePictureRequired.Error()+"\n",
		)
	}
}
