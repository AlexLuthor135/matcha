...
func TestUserBlockHandlersRejectInvalidRequests(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantBody:   RelationshipErrors.InvalidTargetUserID.Error() + "\n",
		},
		{
			name:       "self block",
			...
			call:       (*Handler).BlockUser,
			wantStatus: http.StatusBadRequest,
			wantBody:   RelationshipErrors.CannotBlockSelf.Error() + "\n",
		},
	}
...
func TestUserBlockHandlerMapsServiceErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			serviceErr: RelationshipErrors.UserNotFound,
			wantStatus: http.StatusNotFound,
			wantBody:   "User not found\n",
		},
		{
			name:       "target user missing",
			serviceErr: RelationshipErrors.TargetUserNotFound,
			wantStatus: http.StatusNotFound,
