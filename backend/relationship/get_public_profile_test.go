...
func TestServiceGetPublicProfileRejectsInvalidTargetID(t *testing.T) {
	...
	if !errors.Is(err, RelationshipErrors.InvalidTargetUserID) {
		t.Fatalf("GetPublicProfile() error = %v, want %v", err, RelationshipErrors.InvalidTargetUserID)
	}
}
...
func TestServiceGetPublicProfileMapsMissingUser(t *testing.T) {
	...
	_, err := NewService(repository).GetPublicProfile(
		context.Background(),
		10,
		20,
	)
	if !errors.Is(err, RelationshipErrors.TargetUserNotFound) {
		t.Fatalf("GetPublicProfile() error = %v, want %v", err, RelationshipErrors.TargetUserNotFound)
	}
}
...
func TestServiceGetPublicProfileReturnsCompletedProfile(t *testing.T) {
	...
	result, err := NewService(repository).GetPublicProfile(
		context.Background(),
		wantProfile.ID,
		wantProfile.ID,
	)
...
func TestGetPublicProfileHandlerMapsServiceErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			repositoryErr: RelationshipErrors.UserNotFound,
