...
func TestServiceUserBlocksRejectInvalidInput(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			blockerID:     10,
			blockedUserID: 10,
			wantErr:       RelationshipErrors.CannotBlockSelf,
		},
	}
...
func TestServiceBlockUserDelegatesToRepository(t *testing.T) {
	...
	err := NewService(repository).BlockUser(context.Background(), 10, 20)
...
func TestServiceUnblockUserDelegatesToRepository(t *testing.T) {
	...
	err := NewService(repository).UnblockUser(context.Background(), 10, 20)
	if !errors.Is(err, repositoryError) {
