...
func (h *Handler) SaveProfileDecision(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case errors.Is(err, RelationshipErrors.InvalidTargetUserID):
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	case errors.Is(err, RelationshipErrors.CannotDecideOwnProfile):
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	case errors.Is(err, RelationshipErrors.InvalidProfileDecision):
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	...
	case errors.Is(err, RelationshipErrors.TargetUserNotFound):
		...
		return
	case errors.Is(err, RelationshipErrors.ProfilePictureRequired):
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	case err != nil:
