...
func TestServiceRecordProfileViewRejectsInvalidInput(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			viewerID:     10,
			viewedUserID: 10,
			wantErr:      RelationshipErrors.CannotViewOwnProfile,
		},
	}
...
func TestServiceRecordProfileViewMapsMissingTargetUser(t *testing.T) {
	repository := &fakeUserRepository{
		getCompletionStatusFn: func(context.Context, uint) (bool, error) {
			return false, RelationshipErrors.UserNotFound
		},
	}

	_, err := NewService(repository).RecordProfileView(
		context.Background(),
		10,
		20,
	)
	if !errors.Is(err, RelationshipErrors.TargetUserNotFound) {
		t.Fatalf("RecordProfileView() error = %v, want %v", err, RelationshipErrors.TargetUserNotFound)
	}
}
...
func TestServiceRecordProfileViewReturnsSavedView(t *testing.T) {
	...
	profileView, err := NewService(repository).RecordProfileView(
		context.Background(),
		wantView.ViewerID,
		wantView.ViewedUserID,
	)
