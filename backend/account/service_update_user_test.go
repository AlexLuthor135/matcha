...
func TestServiceUpdateUserWithoutEmailDoesNotRequireEmailSender(t *testing.T) {
	...
	_, err := NewService(repository, nil).UpdateUser(
		context.Background(),
		17,
...
func TestServiceUpdateUserRejectsInvalidEmail(t *testing.T) {
	_, err := NewService(&fakeUserRepository{}, nil).UpdateUser(
		context.Background(),
		17,
		UserUpdateInput{Email: stringPointer("not-an-email")},
	)
	if !errors.Is(err, AccountErrors.InvalidEmail) {
		t.Fatalf("UpdateUser() error = %v, want %v", err, AccountErrors.InvalidEmail)
	}
}
...
func TestServiceUpdateUserWrapsEmailDeliveryError(t *testing.T) {
	...
	if !errors.Is(err, AccountErrors.EmailDeliveryFailed) {
		t.Fatalf("UpdateUser() error = %v, want %v", err, AccountErrors.EmailDeliveryFailed)
	}
}
