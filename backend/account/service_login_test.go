...
func TestServiceLoginWithUserName(t *testing.T) {
	...
	user, err := NewService(repository, nil).Login(
		context.Background(),
		"  test-user  ",
		password,
	)
...
func TestServiceLoginRejectsInvalidInputAndCredentials(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			repository: &fakeUserRepository{},
			want:       AccountErrors.LoginFieldsMissing,
		},
		{
			name:       "blank password",
			login:      "test-user",
		...
		},
		{
			...
			want: AccountErrors.InvalidCredentials,
		},
		{
			name:     "wrong password",
			login:    "test-user",
		...
		},
		{
			...
			want: AccountErrors.EmailNotVerified,
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := NewService(test.repository, nil).Login(
				context.Background(),
				test.login,
				test.password,
			)
