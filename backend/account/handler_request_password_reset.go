...
func (h *Handler) RequestPasswordReset(w http.ResponseWriter, r *http.Request) {
	...
	switch {
	case errors.Is(err, AccountErrors.EmailBlank):
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	case errors.Is(err, AccountErrors.PasswordResetEmailDeliveryFailed):
		log.Printf("Send password reset email: %v", err)
		http.Error(w, AccountErrors.PasswordResetEmailDeliveryFailed.Error(), http.StatusBadGateway)
		return
	case err != nil:
