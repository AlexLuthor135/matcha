...
func TestServiceRegisterRejectsInvalidUserName(t *testing.T) {
	_, err := NewService(&fakeUserRepository{}, nil).Register(
		context.Background(),
		RegisterInput{
	...
	if !errors.Is(err, AccountErrors.InvalidUserName) {
		t.Fatalf("Register() error = %v, want %v", err, AccountErrors.InvalidUserName)
	}
}

func TestServiceUpdateUserRejectsInvalidUserName(t *testing.T) {
	invalidUserName := "user@example.com"
	_, err := NewService(&fakeUserRepository{}, nil).UpdateUser(context.Background(), 17, UserUpdateInput{UserName: &invalidUserName})
	if !errors.Is(err, AccountErrors.InvalidUserName) {
		t.Fatalf("UpdateUser() error = %v, want %v", err, AccountErrors.InvalidUserName)
	}
}
