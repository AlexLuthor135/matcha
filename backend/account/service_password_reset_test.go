...
func TestServiceRequestPasswordResetWrapsDeliveryError(t *testing.T) {
	...
	if !errors.Is(err, AccountErrors.PasswordResetEmailDeliveryFailed) {
		t.Fatalf("RequestPasswordReset() error = %v, want %v", err, AccountErrors.PasswordResetEmailDeliveryFailed)
	}
}
...
func TestServiceResetPasswordHashesTokenAndPassword(t *testing.T) {
	...
	err := NewService(repository, nil).ResetPassword(
		context.Background(),
...
func TestServiceResetPasswordRejectsInvalidInput(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want:  AccountErrors.PasswordResetFieldsMissing,
		},
		{
			name:  "blank password",
		...
		},
		{
			...
			want:  AccountErrors.InvalidPassword,
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			err := NewService(&fakeUserRepository{}, nil).ResetPassword(
				context.Background(),
				test.input,
			)
