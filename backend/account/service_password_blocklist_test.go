...
func TestPasswordBlocklistAppliesToAllPasswordCreationFlows(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			call: func() error {
				return NewService(&fakeUserRepository{}, nil).UpdatePassword(
					context.Background(),
					17,
					"Current1!Password",
					"Password1!",
		...
		},
		{
			name: "password reset",
			call: func() error {
				return NewService(&fakeUserRepository{}, nil).ResetPassword(
					context.Background(),
					ResetPasswordInput{
						Token:       "reset-token",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if err := test.call(); !errors.Is(err, AccountErrors.InvalidPassword) {
				t.Fatalf("password flow error = %v, want %v", err, AccountErrors.InvalidPassword)
			}
		})
	}
}
