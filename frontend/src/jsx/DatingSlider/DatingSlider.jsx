...
export default function DatingSlider() {
    ...
    const markProfile = async (decision) => {
        if (!currentProfile || isSavingDecision) {
            return;
        }
        ...
        try {
            const response = await axiosInstance.put(
                `/backend/api/accounts/profiles/${currentProfile.id}/decision`,
                { decision }
            );
            ...
            setCurrentIndex((previousIndex) => Math.max(
                0,
                Math.min(previousIndex, nextProfiles.length - 1)
            ));

            if (response.data?.is_match) {
                navigate(`/chat?user=${currentProfile.id}`);
            }
        } catch (requestError) {
            console.log("SAVE PROFILE DECISION ERROR", requestError);
            setDecisionError("Could not save profile decision");
        } finally {
    ...
    return(
        <div id="user-profile-container">
            <div id="user-profile" className="dating-profile">
                ...
                {decisionError && (
                    <p className="dating-status">{decisionError}</p>
                )}
                <div className="profile-actions">
                    <Button
                        disabled={isSavingDecision}
                        onClick={() => markProfile("dislike")}
                    >
                        Dislike
                    </Button>
                    <Button
                        disabled={isSavingDecision}
                        onClick={() => markProfile("like")}
                    >
                        Like
                    </Button>
                </div>
