...
The route can still be useful for testing and for user-triggered notification flows.
...
For user-created notifications, use the sender user's ID.
...
### Why Prefer Backend-Generated Notifications?

`POST /api/accounts/notifications/send` is available to any authenticated user.

For product-owned events, prefer creating notifications inside backend handlers with `createSystemNotification`. That keeps match, like, and other notification rules on the server instead of trusting arbitrary client payloads.
